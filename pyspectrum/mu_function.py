...
class MuCount(BaseCount):
	...
	def save_ksampling(self,path_spectrum,path_ksampling=None,kmax=0.1):
		...
		axis = [0,1,2]
		Nmesh = get_data('Nmesh');kfun = get_data('kfun')
		self.logger.info('Used grid {0[0]:d} x {0[1]:d} x {0[2]:d} with fundamental wavevector {1[0]:.4g} x {1[1]:.4g} x {1[2]:.4g}.'.format(Nmesh,kfun))
		halfNmesh = []
		for iaxis in axis:
