...
def total_weight(mesh):

	# the selection
	sel = mesh.compute(mesh[mesh.selection])

	# sum up completeness weights
	wsum = mesh.compute(mesh[sel][mesh.weight].sum())
        
	return mesh.comm.allreduce(wsum)
...
class PyFFT2PCF(object):
	...
	* Scoccimarro, Roman, `Fast estimators for redshift-space clustering`, Phys. Review D, 2015
	...
	def __init__(self, first, poles,
		...
		self.attrs['first.W'] = total_weight(self.first)
		self.attrs['second.W'] = total_weight(self.second)
